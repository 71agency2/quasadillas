...
// LES 4 CATÉGORIES OFFICIELLES SYNCHRONISÉES AVEC L'APP CLIENT
...
export default function AdminDashboard() {
  ...
  const annulerModification = () => {
    ...
    setCategorie(CATEGORIES[0]); setSupplements([]); 
    ...
    <div className="min-h-screen bg-black text-white p-6 font-sans">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8 text-center">
          <h1 className="text-4xl font-extrabold uppercase tracking-wide">Panel Admin - <span className="text-red-600">Food Plus</span></h1>
                  ...
                  return (
                    <div key={cmd.id} className="bg-zinc-950 border border-zinc-800 p-5 rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                      <div className="space-y-1 grow w-full md:w-auto">
                        ...
                        {/* whitespace-pre-wrap permet de bien afficher les retours à la ligne des suppléments */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-1 bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-xl h-fit sticky top-6">
              ...
              <form onSubmit={handleMenuSubmit} className="space-y-4">
                ...
                <div><label className="block text-xs font-bold uppercase text-gray-400 mb-2">Catégorie</label><select value={categorie} onChange={(e) => setCategorie(e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-3 text-white outline-none cursor-pointer">{CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}</select></div>
                ...
                <div className="pt-2 space-y-2">
                  <button type="submit" disabled={uploading} className="w-full bg-red-600 text-white font-bold py-3 rounded-lg uppercase text-sm">{uploading ? "Traitement..." : editingProduit ? "💾 Enregistrer" : "✅ Valider le produit"}</button>
            ...
            <div className="lg:col-span-2 bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-xl">
              ...
              {produits.length === 0 ? <p className="text-gray-500 text-center py-8">Aucun produit.</p> : (
                <div className="space-y-10">
                  {CATEGORIES.map(cat => {
                    const filtered = produits.filter(p => (p.categorie || CATEGORIES[0]) === cat);
